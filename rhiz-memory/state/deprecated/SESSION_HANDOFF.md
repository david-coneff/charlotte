...
- **next_action**: monolith partition is done (above); the `src/` tree is fully modular. Open housekeeping: (a) `web-crawler.html` still lacks the triage/tracker UX; (b) the AD log stops at AD-081 while facade/build comments reference AD-082/083 — write those bodies (or renumber) to close the dangling refs; (c) SYNTHESIS §7 threads. Note the **244/0 test suite is still NOT in the repo** (ephemeral scratchpad), so any *logic-level* (non-code-motion) decomposition needs a restored suite first; the byte-identical build harness only guards pure code motion. (Partitioning the ADR log = done, AD-080; the build = done, AD-082.)
