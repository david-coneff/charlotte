...
<main>
 ...
 <div class="card">
  ...
  ${oosPanel}
  <div class="panel hidden" id="panel-errint">${activeInt.length ? `<p class="muted">Broken internal pages — these are yours to fix.</p>${showPick ? exportBar("errint") + pickHelp + folderHelp + testBar("errint") + domainTools("errint") + domainGroups(activeInt, "errint", errintHead, triageCells, folderOf) : `<div class="tablewrap"><table><thead><tr><th>Broken URL</th><th class="reasoncol">Reason</th><th class="foundcol">Found on</th></tr></thead><tbody>${errRows(activeInt)}</tbody></table></div>`}` : `<p class="muted">No internal errors. 🎉</p>`}</div>
