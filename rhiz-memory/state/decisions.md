...
- **AD-088** (2026-06-28) — Three live-run fixes: (1) discover scope now treats ALL seed hosts as internal (`seedHosts` Set replaces single `start.hostname`), so a multi-URL input doesn't mis-classify a sibling host as external; (2) `buildVerifyCommand()` now hard-codes `--max-pages none` so a large repo with thousands of seed documents isn't silently truncated at 200; (3) discover emits `# discover start` to the log and `processLine` sets `gDiscoverPhase = true` at the subsequent `# crawl start`; note: the counter-reset half of (3) was reverted — see AD-089
- **AD-089** (2026-06-28) — Fix instant "Done — 0 crawled in 0:00": `deleteIfExists(livePath)` silently swallowed deletion failures, leaving a stale `DONE_0` that the first poll read and declared done; replaced with `writeFile(livePath, "")` (FSO ForWriting truncates); `deleteIfExists(errPath)` added to prevent stale fallback display; `# crawl start` handler restored (clears `gDiscoverPhase` only, no counter reset); activity label shows "discovering JS links…" / "browser rendering, waiting for first page…" during the discover phase; delivery was bitten by the Edit-tool smart-quote trap (straight `"` — curly `“”`, JScript "Invalid character"; see SYNTHESIS §5 #31); NOTE — this AD fixed only the false instant-finish, the **Laserfiche zero-crawl symptom remains OPEN** (SYNTHESIS §7)
- **AD-090** (2026-06-28) — Laserfiche "nothing gets crawled": reproduced a concrete cause with the bundled Chromium — the discover browser harvest queried only the top document (`page.$$eval`/`page.evaluate` are main-frame-only), so a WebLink folder listing embedded in an `<iframe>` yielded 0 links —> empty seeds —> crawl.js crawls nothing. Fix: harvest `a[href]` + onclick/data-* from every `page.frames()` (strict superset; skips detached/cross-document frames). Added a loud "rendered N page(s) but found NO documents/child pages" diagnostic (settle/timeout · login/disclaimer/repo-picker · pagination/virtual-scroll · http-fallback) surfaced via the GUI's live-monitor fallback. Verified: iframe fixture 0—>5 docs, real-anchor case unregressed, full chain discover—>seeds—>crawl.js = 6 pages/0 errors. CAVEAT: live DHW site unreachable from dev env (network policy) — the diagnostic covers residual causes (see SYNTHESIS §5 #32, §7). CONFIRMED 2026-06-28 — the operator's live re-run against the DHW WebLink repository crawls again
- **AD-091** (2026-06-28) — Multi-seed host classification: crawl.js labeled a sibling SEED host (`publicdocuments.dhw.idaho.gov`, the WebLink repo) as **External** because it scoped internal to a single `startHost` (`startUrls[0]`). AD-088 fixed this in crawl-render.js (discover) but not in the twin classifier in crawl.js (verify). Fix: build an `internalHosts` Set from the seed URLs and test membership (`isInternalHost`), scoped to seedMode so single-site and multi-site-index modes are unchanged. Reproduced with a one-server/two-hostname fixture (localhost+127.0.0.1): sibling seed's links 0int/2ext—>2int/0ext, 3—>5 pages, externalLinks 3—>0; single-site control still 2 external (no regression). Root crawl.js rebuilt from src. Completes AD-088; durable trap SYNTHESIS §5 #33 (sibling-classifier drift)
- **AD-092** (2026-06-29) — "Stops at 0" regression: a stale `crawl-gui-stop.flag` (from a prior Stop click) makes the discover phase render 0 pages and write near-empty seeds —> nothing crawled. The flag is cleared by `deleteIfExists`, which silently swallows a locked-file failure (a locked file; specific locker never confirmed) — the SAME silent-deleteIfExists class as AD-089, which had hardened only the live log. Fix: `freshFlag()` clears the flag before the bat embeds its path and, if it survives deletion, switches the run to a uniquely-named flag the stale one can't shadow (startCrawl + recheckLinks). Ruled out the other 0-crawl causes first (HTA parses clean per §5 #31; writeFile(livePath,"") intact; crawl.js + discover both crawl in local repro). GUI-only; tokenizer-verified parse-clean. Generalizes AD-089; durable trap §5 #34. Workaround without the fix: delete the .flag files
- **AD-093** (2026-06-29) — The REAL "stops at 0": the operator's `crawl-progress.log` showed a `DONE_0` ABOVE the live `# extcheck` lines — a STALE marker (extcheck is crawl.js's post-crawl phase; the bat writes DONE_ only at the end). `resetLiveState()` reset `gPos=0`, so the first poll read the stale `DONE_0` before the new run wrote anything (discover still launching the browser) and declared "Done — 0 crawled" while the crawl actually ran. AD-089's `writeFile(livePath,"")` truncate isn't reliably clearing it (root cause UNCONFIRMED; the OneDrive guess was retracted — not running). Fix: seek `gPos` to the END of the live log at launch so the reader consumes only post-launch content — robust whether or not truncation took. Verified by simulating readNew/gPos. Supersedes AD-092 (disproven) as the cause; durable trap §5 #35 (harden the CONSUMER against stale state, not just the producer-side cleanup)
- **AD-094** (2026-06-29) — Rebuild/re-check ORPHANED all triage: the report's `startHost` was re-derived from `pages[0].url` (first crawled page) because the JSON never stored the original start URL. An apex—>www redirect makes `pages[0]` a `www` URL, so the rewrite flipped `HOST` apex—>www — and `HOST` is the triage localStorage namespace AND the verdict-import gate, so every verdict went invisible + the import was rejected. Fix: record `startUrl` in the JSON + derive host from it; honor explicit `--start-url` for old JSONs; GUI passes its Start URL on Rebuild/Re-check. New crawls auto-preserve host; old reports recover by re-rebuilding with the Start URL set. Durable trap §5 #36 (store identity, don't re-derive it from drift-prone data)
