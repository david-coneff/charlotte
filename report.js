...
function buildReport(state, cfg, allow, partial) {
  ...
  const brokenN = (id, count, denom) => `<span id="${id}">${count.toLocaleString()}</span>${denom > 0 ? ` <span class="pct">(${Math.round((count / denom) * 100)}%)</span>` : ""}`;
