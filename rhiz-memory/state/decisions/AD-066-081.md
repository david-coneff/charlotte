...
live-DOM `a[href]` harvest reaches them with no onclick tuning needed. Note: `crawl-render.js` reads CLI
flags only (no config file), so discover's `--max-pages`/`--max-depth` must be set on its own command line.
